...
games(Ps, T, K, Gs, P):-
    ...
    findall( [Gs,P] , solution(Ps, T, K, Gs, P), L),

    /* Seperate the list with the combinations of games and its pleasure */
    seperator(L, Times, Pleasure),

    /* Find the max pleasure from the given list Pleasre */
    maxlist2(Pleasure, M),

    /* Create the list with the combinations of the games that have the max pleasure*/
    maxPT(Pleasure, Times, _, FinalTimes, M),

    /* Return each combination of games one by one */
    delete(Gs,FinalTimes, _),

    /* The max pleasure is at the variable M */
    P = M.
